...
from flask import Flask, render_template, request, jsonify, session, json, url_for
...
def is_login():
    ...
    if 'userid' in session and 'loggedin' in session:
        ...
        query = {"_id": ObjectId(session['userid'])}
...
def blog():
    if is_login:
        ...
        results = post_collection.find().sort("_id", -1)
...
def create_post(content):
    if content and is_login:
        post_collection = mongo.db.users_post
        obj = post_collection.insert_one({'content': content, 'user': session["userid"]})
